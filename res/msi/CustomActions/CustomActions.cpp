    ...
    __in MSIHANDLE hInstall)
{
    ...
    hr = WcaInitialize(hInstall, "CopyTomlConfig");
    ExitOnFailure(hr, "Failed to initialize");
    ...
    pwz = pwzData;
    hr = WcaReadStringFromCaData(&pwz, &msiPath);
    ExitOnFailure(hr, "failed to read msi path from custom action data");

    hr = WcaReadStringFromCaData(&pwz, &installFolder);
    ExitOnFailure(hr, "failed to read install folder from custom action data");

    if (msiPath == NULL || msiPath[0] == L'\0' || installFolder == NULL || installFolder[0] == L'\0')
    {
        WcaLog(LOGMSG_STANDARD, "CopyTomlConfig: empty path, skipping.");
        hr = S_OK;
        goto LExit;
    }

    WCHAR msiDir[MAX_PATH];
    WCHAR srcToml[MAX_PATH];
    WCHAR dstToml[MAX_PATH];

    if (FAILED(StringCchCopyW(msiDir, ARRAYSIZE(msiDir), msiPath)) || !PathRemoveFileSpecW(msiDir))
    {
        WcaLog(LOGMSG_STANDARD, "CopyTomlConfig: failed to derive msi directory, skipping.");
        hr = S_OK;
        goto LExit;
    }
    ...
    if (CopyFileW(srcToml, dstToml, FALSE))
    {
        WcaLog(LOGMSG_STANDARD, "CopyTomlConfig: copied %ls -> %ls", srcToml, dstToml);
    }
    else
    {
        WcaLog(LOGMSG_STANDARD, "CopyTomlConfig: CopyFileW failed, err=%lu", GetLastError());
    }

...
pub static ref T: std::collections::HashMap<&'static str, &'static str> =
    [
        ...
        ("Checking...", "檢測中..."),
        ("Required", "必填"),
        ("Multiple server config", "多伺服器配置"),
        ("Auto switch server", "自動切換伺服器"),
        ("Add server config", "新增伺服器配置"),
        ("Edit server config", "編輯伺服器配置"),
        ("Delete server config", "刪除伺服器配置"),
        ("Delete server config tip", "刪除伺服器配置 {name}？"),
        ("No server config", "暫無伺服器配置"),
        ("Not checked", "未檢測"),
        ("Check availability", "檢測可用性"),
        ("ID Server Port", "ID 伺服器連接埠"),
        ("Relay Server Port", "中繼伺服器連接埠"),
        ("Key", ""),
        ("Low power mode", "低功耗模式"),
        ("Enable low-power mode on dual-GPU Macs", "在雙顯卡 Mac 上啟用低功耗模式"),
        ("low_power_mode_tip", "針對雙顯卡 Mac：透過降低擷取幀率與解析度並停用硬體編解碼，來減少發熱與耗電，避免喚醒獨立顯示卡。於新連線或重新啟動後生效，CPU 使用率會上升。"),
        ("Open config folder", "開啟配置檔所在目錄"),
        ("Config name is required", "設定名稱不能為空"),
        ("Config name cannot exceed 50 characters", "設定名稱不能超過 50 個字元"),
        ("ID server address is required", "ID 伺服器位址不能為空"),
        ("Server address is required", "伺服器位址不能為空"),
        ("Invalid ID server port", "ID 伺服器連接埠範圍無效"),
        ("Invalid relay server port", "中繼伺服器連接埠範圍無效"),
        ("Duplicate config name", "設定名稱已存在"),
        ("This ID server address already exists", "此 ID 伺服器設定已存在"),
        ("Maximum of 5 configs reached", "已達到最大設定數量（5 個）"),
        ("The last config cannot be deleted", "不能刪除最後一個設定"),
        ("The default config cannot be deleted", "預設設定不允許刪除"),
        ("The default config is pinned to the top", "預設設定固定置頂，不可調整優先順序"),
        ("Invalid priority position", "優先順序位置無效"),
        ("Config not found", "設定不存在"),
        ("The ID server is unreachable", "ID 伺服器無法連線"),
        ("Disconnect the remote session before switching config", "請先中斷遠端連線再切換設定"),
        ("Switching is protected, try again later", "切換保護中，請稍後重試"),
        ("Move up", "上移"),
        ("Move down", "下移"),
    ].iter().cloned().collect();
}
